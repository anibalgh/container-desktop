      ...
      <div className="flex-1 overflow-auto rounded-lg border" style={{ borderColor: "var(--color-border)" }}>
        <table className="w-full text-sm">
          <thead><tr style={{ backgroundColor: "var(--color-surface-secondary)" }}>
            ...
            <STh col="mountpoint" cCol={col as string} dir={dir} label="Mountpoint" onClick={() => toggle("mountpoint")} />
            <STh col="created" cCol={col as string} dir={dir} label="Created" onClick={() => toggle("created")} />
          ...
          <tbody>
            {sorted.length === 0 ? <tr><td colSpan={5} className="px-4 py-12 text-center" style={{ color: "var(--color-text-muted)" }}>No volumes found.</td></tr> :
              sorted.map((v) => (
                <tr key={v.name} className="border-t" style={{ borderColor: "var(--color-border)" }}>
                  ...
                  <td className="px-4 py-2.5"><button onClick={() => doRemove(v.name)} className="px-2 py-1 text-xs font-medium rounded border hover:opacity-80" style={{ borderColor: "var(--color-danger)", color: "var(--color-danger)" }}>Remove</button></td>
                </tr>
              ))}
          </tbody>
        </table>
      ...
      {showCreate && (
        <div className="fixed inset-0 flex items-center justify-center z-50" style={{ backgroundColor: "rgba(0,0,0,0.5)" }}>
          <div className="rounded-lg p-6 max-w-sm w-full mx-4 shadow-xl" style={{ backgroundColor: "var(--color-surface)" }}>
            ...
            <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Volume name" className="w-full px-3 py-2 text-sm rounded-md border mb-4" disabled={creating} style={{ borderColor: "var(--color-border)", backgroundColor: "var(--color-surface-secondary)", color: "var(--color-text)" }} />
