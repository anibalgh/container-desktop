            ...
            {sorted.length === 0 ? <tr><td colSpan={6} className="px-4 py-12 text-center" style={{ color: "var(--color-text-muted)" }}>{t.networks.empty}</td></tr> :
              sorted.map((n) => (
                <tr key={n.id} className="border-t" style={{ borderColor: "var(--color-border)" }}>
                  ...
                  <td className="px-4 py-2.5"><button onClick={() => doRemove(n.id)} className="px-2 py-1 text-xs font-medium rounded border hover:opacity-80" style={{ borderColor: "var(--color-danger)", color: "var(--color-danger)" }}>{t.common.remove}</button></td>
                </tr>
              ))}
          </tbody>
        </table>
