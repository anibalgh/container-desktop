...
export async function containerLogs(
  ...
  return invoke("container_logs", { id, tail, follow });
}

export function onContainerLogLine(
  callback: (line: LogLine) => void,
): Promise<UnlistenFn> {
  return listen<LogLine>("container-log-line", (event) => {
...
export function onImagePullProgress(
  callback: (msg: string) => void,
): Promise<UnlistenFn> {
  return listen<string>("image-pull-progress", (event) => {
    callback(event.payload);
  });
}
...
export function onComposeOutput(
  ...
  return listen<LogLine>("compose-output", (event) => {
    callback(event.payload);
  });
}
...
export function onDockerConnected(
  callback: (info: DockerInfo) => void,
): Promise<UnlistenFn> {
  return listen<DockerInfo>("docker-connected", (event) => {
    callback(event.payload);
  });
}

export function onDockerError(
  callback: (error: string) => void,
): Promise<UnlistenFn> {
  return listen<string>("docker-error", (event) => {
    callback(event.payload);
  });
}
